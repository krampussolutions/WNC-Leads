...
export default async function DashboardPage() {
  ...
  // Only show quote requests for *your* listing (prevents showing everyone's quotes)
  const { data: quotes } = listing?.id
    ? await supabase
        .from("quote_requests")
  ...
  return (
    <>
      ...
      <main className="mx-auto max-w-6xl px-6 py-12">
        ...
        <div className="mt-8 grid gap-6 md:grid-cols-3">
          <Card>
            ...
            {!active ? (
              <div className="mt-4">
                <Link href="/pricing">
                  <Button>Subscribe to publish</Button>
                </Link>
              </div>
            ) : (
              <p className="mt-3 text-sm text-slate-300">
                Active subscription — your listing can be published.
              </p>
            )}
          </Card>
          ...
          <Card>
            ...
            <div className="mt-4">
              <Link href="/dashboard/listing">
                <Button variant="secondary">{listing ? "Edit listing" : "Create listing"}</Button>
              </Link>
            </div>
          </Card>
                ...
                <div className="flex flex-col gap-2 md:flex-row md:items-start md:justify-between">
                  <div>
                    ...
                    <div className="text-sm text-slate-300">
                      {q.requester_email}
                      {q.requester_phone ? ` · ${q.requester_phone}` : ""}
                    </div>
        ...
        {/* ✅ Reviews panel is now actually rendered */}
        <div className="mt-10">
          <ReviewsPanel />
        </div>
      </main>
